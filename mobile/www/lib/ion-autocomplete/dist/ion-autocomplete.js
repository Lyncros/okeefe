    ...
    function ($ionicBackdrop, $ionicScrollDelegate, $document, $q, $parse, $interpolate, $ionicPlatform, $compile, $templateRequest) {
        return {
            ...
            controller: ['$attrs', '$timeout', '$scope', function ($attrs, $timeout, $scope) {
                ...
                $timeout(function () {
                    controller.placeholder = valueOrDefault(controller.placeholder, 'Click para ingresar valor...');
                    controller.cancelLabel = valueOrDefault(controller.cancelLabel, 'Hecho');
                    controller.selectItemsLabel = valueOrDefault(controller.selectItemsLabel, "Selecciona una opción...");
                    controller.selectedItemsLabel = valueOrDefault(controller.selectedItemsLabel, $interpolate("Opciones seleccionadas{{maxSelectedItems ? ' (max. ' + maxSelectedItems + ')' : ''}}:")(controller));
            ...
            link: function (scope, element, attrs, controllers) {
                ...
                var template = [
                    ...
                    '<button style="color:#000;" class="ion-autocomplete-cancel button button-clear" ng-click="viewModel.cancelClick()">{{viewModel.cancelLabel}}</button>',
                    '</div>',
