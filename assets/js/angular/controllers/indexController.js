        ...
        .controller('indexController',
            function ($route, $scope, $rootScope, $window, $location, $uibModal, entitiesService, defaultFactory, $auth, userService, okeefeApiService, searchApiService) {
                ...
                $scope.getLocation = function (val) {
                    return searchApiService.searchApi.readLocations(entitiesService.idOperacion($scope.searchParam.oper),$scope.searchParam.tipo,val).then(function (response) {
                ...
                $scope.searchProp = function () {
                    if ($scope.validateForm()) {
                        window.location = '#/propiedades/'+$scope.searchParam.property+'/'+$scope.searchParam.oper+'?ubicacion=' + $scope.searchParam.zona + '&emp=' + ($scope.searchParam.empr || 0);
                    }
                };
